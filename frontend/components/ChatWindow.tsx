...
export default function ChatWindow() {
  ...
  useEffect(() => {
    listScenarios()
      .then(setScenarios)
      .catch(() => setScenarios([])); // pas bloquant : le chat marche sans scénario
  }, []);
  ...
  async function submitMessage(text: string) {
    ...
    try {
      const response = await sendMessage(trimmed, conversationId, selectedScenarioId);
  ...
  return (
    <div style={{ maxWidth: 700, margin: "0 auto", fontFamily: "sans-serif" }}>
      {scenarios.length > 0 && conversationId === null && (
        <div
          style={{
            marginBottom: "1rem",
            padding: "0.75rem",
          ...
          >
            ...
            {scenarios.map((s) => (
              <option key={s.id} value={s.id}>
                {s.titre} {s.niveau_cecrl ? `(${s.niveau_cecrl})` : ""}
              </option>
            ))}
          </select>
        </div>
